...
const std::vector<std::string> serial_prefixes = {"/dev/ttyACM"};
...
int serialOpen_ (const char *device, const int baud)
{
...
int Serial::connect_serial() {
    ...
    for (const std::string serial_prefix: serial_prefixes) {
        for (int i = 0; i < max_tries; i++) {
            ...
            m_fd = serialOpen_(serial_interface.c_str(), m_baudrate);
            if (m_fd != -1) { 
